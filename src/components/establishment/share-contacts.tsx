...
export function ShareContacts({
  ...
  const shareText = `Agende seu horário na ${name}:`
  const fullShareText = `${shareText} ${pageUrl}`

  const whatsappContactUrl = `https://wa.me/55${whatsapp.replace(/\D/g, "")}`
  const whatsappShareUrl = `https://wa.me/?text=${encodeURIComponent(fullShareText)}`
  const facebookShareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(pageUrl)}`

  async function copyAndToast(label?: string) {
    try {
      await navigator.clipboard.writeText(pageUrl)
      if (label) {
        toast.success(`Link copiado! Cole no ${label}.`)
      } else {
        setCopied(true)
    ...
    <div className="mt-4 flex items-center justify-between gap-3">
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {tiktok && (
          <Button
            ...
            title={`@${tiktok}`}
          >
            <a href={`https://tiktok.com/@${tiktok}`} target="_blank" rel="noreferrer">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="min-w-48">
          ...
          <DropdownMenuItem onClick={() => copyAndToast("Instagram")}>
            <InstagramIcon className="size-4 text-[#E4405F]" />
            Instagram
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => copyAndToast("TikTok")}>
            <TikTokIcon className="size-4" />
            TikTok
          </DropdownMenuItem>
          <DropdownMenuItem onClick={() => copyAndToast("YouTube")}>
            <YouTubeIcon className="size-4 text-[#FF0000]" />
            YouTube
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => copyAndToast()}>
            {copied ? (
